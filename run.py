...
def run_simulation_server(name, monitor_overview):
    if name == "diag0":
        ...
        devices = load_relevant_controls(
            os.path.join(FILEPATH, "simulation_server", "yaml_configs", "DL1_2_OTR2.yaml"))
    else:
        raise ValueError(f"Unknown virtual accelerator name: {name}")

    PVDB = create_pvdb(devices)

    va = get_virtual_accelerator(name, monitor_overview)
...
if __name__ == "__main__":
    ...
    run_simulation_server(args.name, args.monitor_overview)
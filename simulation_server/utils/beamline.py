...
def create_beamline(devices: dict, screen_name: str, **params):
    ...
    for key_num, key in enumerate(keys):
        if "QUAD" in key:
            l_eff = devices[key].get("metadata", {}).get("l_eff", 0.01)
            madname = devices[key].get("madname", f"missing_quadname_{key_num}")
            quad = Quadrupole(
                name=madname, k1=torch.tensor(0.0), length=torch.tensor(l_eff)
            )
            ...
            screen = Screen(
                name=madname,
                resolution=(params.get("nrow", 600), params.get("ncol", 800)),
                is_active=True,
            )
            segment_list.append(screen)
        elif "TCAV" in key:
            TransverseDeflectingCavity(
                length=torch.tensor(0.8),
                voltage=torch.tensor(350000),
                phase=torch.tensor(0),
                frequency=torch.tensor(2856 * 1e6),
            )
